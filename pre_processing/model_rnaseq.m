...
reads = quantile_normalize(RNAseq.reads);
filt = find(~all(isnan(reads)));
npcs = size(dMat_f_n_p_n_c,2);
[~,x] = pca(reads(:,filt)','NumComponents',npcs);

nfields = size(dMat_f_n_p_n_c,2);
lin_rvals = cell(nfields,1);
lin_shuf_rvals = cell(nfields,1);
[~,y_obs] = pca(dMat_f_n_p_n_c,'NumComponents',npcs);
[~,y_shuf] = pca(shuffle_columns(dMat_f_n_p_n_c),'NumComponents',npcs);
...
%     y_shuf = distilled_shuf_data;
    ...
    for i=k:size(x,2)
        ...
        canon_val_p(ct,1:8) = rnd_canon_stats.p(1:10);
        [~,~,rnd_canon_r,~,~,rnd_canon_stats] = canoncorr(x,y_shuf(:,1:i));
        shuf_val_p(ct,1:8) = rnd_canon_stats.p(1:10);
        ct = ct+1;
    end
