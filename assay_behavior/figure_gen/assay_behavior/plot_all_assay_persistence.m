function plot_all_assay_persistence(data,fields)
% Parses assay persistence data into unique combinations of assay x metric
% across days of testing. The correlation of each metric to itself across days
% of testing is computed as a function of the number of days between
% measurements. Where multiple measurements are taken with the same number of
% days between testing, the average is computed. All persistence measurements
% are then plotted by assay.

% group measures by assay
fields = standardize_fieldnames(fields);
...
for i=1:numel(unique_assays)
   ...
   assay_grouped_fields{i} = fields(assay_mask);
end
...
% generate an r-value plot for each assay
% with a line for each metric as a function of days between
figure;
ah=subplot(2,3,1);
ah2 = subplot(2,3,2);
